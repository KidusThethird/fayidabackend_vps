...
router.get("/", async (req, res, next) => {
  try {
    const CitySelected = await prisma.MaterialLink.findMany({
      //   orderBy: {
      //     cityName: "asc",
      //   },
    });
...
router.get("/:id", async (req, res, next) => {
  try {
    ...
    const CitySelected = await prisma.MaterialLink.findUnique({
      where: {
...
router.post("/", checkAuthenticated, async (req, res, next) => {
  ...
  try {
    const CitySelected = await prisma.MaterialLink.create({
      data: req.body,
    });
    res.json(CitySelected);
  } catch (error) {
...
router.patch("/:id", checkAuthenticated, async (req, res, next) => {
  if (req.isAuthenticated()) {
    if (req.user.accountType == "Admin" || req.user.accountType == "SubAdmin") {
      try {
        const { id } = req.params;
        const CitySelected = await prisma.MaterialLink.update({
          where: {
            id: req.params.id,
          },
          data: req.body,
...
router.delete("/:id", checkAuthenticated, async (req, res, next) => {
  if (req.isAuthenticated()) {
    if (req.user.accountType == "Admin" || req.user.accountType == "SubAdmin") {
      try {
        const { id } = req.params;
        CitySelected = await prisma.MaterialLink.delete({
          where: {
            id: parseInt(req.params.id),
          },
        });

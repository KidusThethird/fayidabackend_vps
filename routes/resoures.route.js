...
router.post(
  ...
  async (req, res) => {
    ...
    let olderFileName = "";
    const singleVideo = await prisma.resources.findUnique({
      where: {
        id: req.params.id,
      },
    });
    ...
    if (req.isAuthenticated()) {
      if (req.user.accountType == "Admin") {
        try {
          ...
          deleteVideo = await prisma.resources.delete({
            where: {
              id: id,
            },
          });
...
router.post("/", async (req, res, next) => {
  try {
    const video = await prisma.resources.create({
      data: req.body,
    });
    res.json(video);
  } catch (error) {}
